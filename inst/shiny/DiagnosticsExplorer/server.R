shiny::shinyServer(function(input, output, session) {
  ...
  observeEvent(eventExpr = getUserSelection(),
               handlerExpr = {
                 data <- consolidationOfSelectedFieldValues(input = input,
                                                            ...
                                                            conceptSetExpressionLeft = getConceptSetExpressionLeft(),
                                                            conceptSetExpressionRight = conceptSetExpressionright(),
                                                            database = database,
  ...
  getSelectedRowsInCohortTableOfCohortDefinitionTab <- shiny::reactive({
  ...
  getCirceRenderedExpressionDetails <- shiny::reactive(x = {
    ...
    selectionsInCohortTable <- getSelectedRowsInCohortTableOfCohortDefinitionTab()
  ...
  getCountsForSelectedCohortsRight <- shiny::reactive(x = {
    data <- cohortCount %>%
      dplyr::filter(.data$cohortId %in% consolidatedSelectedFieldValue()$cohortIdRight) %>% 
      dplyr::select(.data$databaseId,
                    .data$cohortSubjects,
                    .data$cohortEntries) %>%
      dplyr::arrange(.data$databaseId)
  ...
  getCountsForSelectedCohortsLeftFilteredToDatabaseId <-
    shiny::reactive(x = {
      selectedDatabase <- getSelectedDatabaseForConceptSetLeft()
      databaseId <- database %>%
        dplyr::filter(
          .data$databaseIdWithVocabularyVersion == selectedDatabase$databaseIdWithVocabularyVersion
        ) %>%
  ...
  getConceptSetExpressionLeft <- shiny::reactive(x = {
    if (all(!doesObjectHaveData(consolidatedSelectedFieldValue()$cohortIdLeft),
            !doesObjectHaveData(consolidatedSelectedFieldValue()$conceptSetIdLeft))) {
      return(NULL)
    }
    conceptSetExpression <- conceptSets %>% 
      dplyr::filter(.data$cohortId %in% consolidatedSelectedFieldValue()$cohortIdLeft) %>% 
      dplyr::filter(.data$conceptSetId %in% consolidatedSelectedFieldValue()$conceptSetIdLeft)
    if (!doesObjectHaveData(conceptSetExpression)) {
      return(NULL)
    }
    data <- getConceptSetDataFrameFromConceptSetExpression(conceptSetExpression)
    return(data)
  })
  
  ###getConceptSetExpressionRight----
  getConceptSetExpressionRight <- shiny::reactive(x = {
    if (all(!doesObjectHaveData(consolidatedSelectedFieldValue()$cohortIdRight),
            !doesObjectHaveData(consolidatedSelectedFieldValue()$conceptSetIdRight))) {
      return(NULL)
    }
    conceptSetExpression <- conceptSets %>% 
      dplyr::filter(.data$cohortId %in% consolidatedSelectedFieldValue()$cohortIdRight) %>% 
      dplyr::filter(.data$conceptSetId %in% consolidatedSelectedFieldValue()$conceptSetIdRight)
    if (!doesObjectHaveData(conceptSetExpression)) {
      return(NULL)
    }
    data <- getConceptSetDataFrameFromConceptSetExpression(conceptSetExpression)
  ...
  getSimplifiedInclusionRuleResultsLeft <- shiny::reactive(x = {
    if (any(!doesObjectHaveData(consolidatedSelectedFieldValue()$cohortIdLeft),
    ...
    data <-
      getResultsInclusionRuleStatistics(dataSource = dataSource,
                                        cohortIds = consolidatedSelectedFieldValue()$cohortIdLeft,
                                        databaseIds = consolidatedSelectedFieldValue()$databaseIdLeft)
    ...
    data <- data %>%
      dplyr::filter(.data$databaseId %in% consolidatedSelectedFieldValue()$databaseIdLeft) %>%
      dplyr::filter(.data$cohortId %in% consolidationOfSelectedFieldValues()$cohortIdLeft)
    return(data)
  })
  
  ###getSimplifiedInclusionRuleResultsRight----
  getSimplifiedInclusionRuleResultsRight <- shiny::reactive(x = {
    if (any(!doesObjectHaveData(consolidatedSelectedFieldValue()$cohortIdRight),
    ...
    data <-
      getResultsInclusionRuleStatistics(dataSource = dataSource,
                                        cohortIds = consolidatedSelectedFieldValue()$cohortIdRight,
                                        databaseIds = consolidatedSelectedFieldValue()$databaseIdRight)
    data <- data %>%
      dplyr::filter(.data$databaseId %in% consolidatedSelectedFieldValue()$databaseIdRight()) %>%
      dplyr::filter(.data$cohortId %in% consolidationOfSelectedFieldValues()$cohortIdRight)
    return(data)
  })
  ...
  output$nameOfSelectedCohortInCohortDefinitionTableLeft <-
    shiny::renderUI(expr = {
      if (!doesObjectHaveData(consolidatedSelectedFieldValue()$cohortIdLeft)) {
        return(NULL)
      }
  ...
  output$nameOfSelectedCohortInCohortDefinitionTableRight <-
    shiny::renderUI(expr = {
      if (!doesObjectHaveData(consolidatedSelectedFieldValue()$cohortIdRight)) {
        return(NULL)
      }
  ...
  output$isDatabaseIdFoundForSelectedCohortCountLeft <-
    shiny::reactive(x = {
      return(!is.null(consolidatedSelectedFieldValue()$cohortIdLeft))
    })
  shiny::outputOptions(x = output,
  ...
  output$simplifiedInclusionRuleTableForSelectedCohortCountLeft <-
    DT::renderDataTable(expr = {
      if (any(
        is.null(consolidatedSelectedFieldValue()$cohortIdLeft),
        is.null(consolidatedSelectedFieldValue()$databaseIdLeft)
      )) {
        return(NULL)
      }
      ...
      cohortCounts <- table %>%
        ...
        dplyr::filter(.data$cohortId == consolidatedSelectedFieldValue()$cohortIdLeft) %>%
        dplyr::filter(.data$databaseId %in% consolidatedSelectedFieldValue()$databaseIdLeft) %>%
        ...
        shiny::conditionalPanel(
          ...
          shiny::tabsetPanel(
            ...
            shiny::tabPanel(
              ...
              tags$br(),
              shiny::conditionalPanel(
                condition = "output.isConceptSetExpressionPresentInSelectedCohortLeft == true",
                shinydashboard::box(
                  title = shiny::textOutput(outputId = "conceptSetExpressionLeftPanelTitle"),
                  ###!!!! selected concept set name + cohort information
                  width = NULL,
                  solidHeader = FALSE,
                  collapsible = TRUE,
                  collapsed = FALSE,
                  shiny::conditionalPanel(condition = "output.isConceptSetExpressionPresentInSelectedCohortLeft == true",
                                          tags$table(
                                            tags$tr(
                                              tags$td(
        ...
        shiny::conditionalPanel(
          ...
          shiny::tabsetPanel(
            ...
            shiny::tabPanel(
              ...
              tags$br(),
              shiny::conditionalPanel(
                condition = "output.isConceptSetExpressionPresentInSelectedCohortRight == true",
                shinydashboard::box(
                  title = shiny::textOutput(outputId = "conceptsetExpressionRightPanelTitle"),
                  solidHeader = FALSE,
                  width = NULL,
                  collapsible = TRUE,
                  collapsed = FALSE,
                  shiny::conditionalPanel(condition = "output.isConceptSetExpressionPresentInSelectedCohortRight == true",
                                          tags$table(
                                            tags$tr(
                                              tags$td(
                                                shinyWidgets::pickerInput(
  ...
  getDetailsForSelectedConceptId <- shiny::reactive(x = {
    ...
    conceptCooccurrence <- getResultsConceptCooccurrence(dataSource = dataSource,
  ...
  #output: isConceptSetExpressionPresentInSelectedCohortLeft----
  output$isConceptSetExpressionPresentInSelectedCohortLeft <-
    shiny::reactive(x = {
      return(!is.null(getConceptSetExpressionLeft()))
    })
  shiny::outputOptions(x = output,
                       name = "isConceptSetExpressionPresentInSelectedCohortLeft",
                       suspendWhenHidden = FALSE)
  ...
  output$isDatabaseIdFoundForSelectedCohortCountRight <-
    shiny::reactive(x = {
      return(!is.null(consolidatedSelectedFieldValue()$databaseIdRight))
    })
  shiny::outputOptions(x = output,
  ...
  output$simplifiedInclusionRuleTableForSelectedCohortCountRight <-
    DT::renderDataTable(expr = {
      if (any(
        is.null(consolidatedSelectedFieldValue()$cohortIdRight),
        is.null(consolidatedSelectedFieldValue()$databaseIdRight)
      )) {
        return(NULL)
      }
      ...
      cohortCounts <- table %>%
        dplyr::inner_join(cohortCount,
                          by = c("cohortId", "databaseId")) %>%
        dplyr::filter(.data$cohortId == consolidatedSelectedFieldValue()$cohortIdRight) %>%
        dplyr::filter(.data$databaseId %in% consolidatedSelectedFieldValue()$databaseIdLeft) %>%
        dplyr::select(.data$cohortSubjects) %>%
      ...
      output$cohortDefinitionTextRight <- shiny::renderUI(
        expr = {
      ...
      #output: conceptsetExpressionRightPanelTitle----
      output$conceptsetExpressionRightPanelTitle <-
        shiny::renderText(
          expr = {
      ...
      ##output: isConceptSetExpressionPresentInSelectedCohortRight----
      output$isConceptSetExpressionPresentInSelectedCohortRight <-
        shiny::reactive(x = {
          return(!is.null(getConceptSetExpressionRight()))
        })
      shiny::outputOptions(x = output,
                           name = "isConceptSetExpressionPresentInSelectedCohortRight",
                           suspendWhenHidden = FALSE)
      ...
      output$conceptsetExpressionJsonRight <- shiny::renderText({
        conceptSetExpression <- conceptSets %>%
          dplyr::filter(
            .data$cohortId %in% consolidatedSelectedFieldValue()$cohortIdRight
          ) %>%
          dplyr::filter(
            .data$conceptSetId %in% consolidatedSelectedFieldValue()$conceptSetidRight
          ) %>%
      ...
      getConceptSetIds <- shiny::reactive(x = {
        if (any(
          is.null(input$conceptSetMultiSelectDropdown),
      ...
      getCohortCountDataSubjectRecord <- shiny::reactive(
        x = {
          data <- getCohortCountDataForSelectedDatabaseIdsCohortIds()
          if (any(is.null(data),
                  nrow(data) == 0))
      ...
      getCohortCountDataSubject <- shiny::reactive(
        x = {
          data <- getCohortCountDataForSelectedDatabaseIdsCohortIds()
          if (any(is.null(data),
                  nrow(data) == 0))
      ...
      getCohortCountDataRecord <- shiny::reactive(
        x = {
          data <- getCohortCountDataForSelectedDatabaseIdsCohortIds()
          if (any(is.null(data),
                  nrow(data) == 0))
          ...
          for (i in 1:length(databaseIds))
          {
            dataTable <- DT::formatStyle(
              ...
              background = DT::styleColorBar(c(0, max(
                table[, i * 2], na.rm = TRUE)), "lightblue"),
              backgroundSize = "98% 88%",
              backgroundRepeat = "no-repeat",
              backgroundPosition = "center"
            )
      ...
      output$doesSelectedRowInCohortCountTableHaveCohortId <- reactive({
        return(!is.null(
          getCohortIdFromSelectedRowInCohortCountTable()
        ))
      })
      ...
      getFixedTimeSeriesTsibble <- reactive({
        if (input$tabs == "timeSeries")
        {
          ...
          if (any(length(
            getCohortIdsFromSelectedCompoundCohortNames()
          ) == 0))
          {
            return(NULL)
          }
      ...
      shiny::observe({
        ...
        calendarIntervalFirstLetter <-
          tolower(substr(
            input$timeSeriesAggregationPeriodSelection, 1, 1
          ))
        data <- getFixedTimeSeriesTsibble()
        if (is.null(data))
        {
      ...
      output$timeDistributionTable <- DT::renderDataTable(
        expr = {
      ...
      getIndexEventBreakdownData <- shiny::reactive(
        x = {
      ...
      shiny::observe({
        if (all(
          !is.null(getIndexEventBreakdownData()),
      ...
      getIndexEventBreakdownDataEnhanced <- shiny::reactive(
        x = {
      ...
      getIndexEventBreakdownDataFiltered <- shiny::reactive(
        x = {
      ...
      getIndexEventBreakdownDataLong <- shiny::reactive(
        x = {
      ...
      getIndexEventBreakdownDataWide <- shiny::reactive(
        x = {
      ...
      getIndexEventBreakdownDataTable <- shiny::reactive(
        x = {
          data <- getIndexEventBreakdownDataLong()
          if (any(is.null(data),
                  nrow(data) == 0))
      ...
      getVisitContextDataLong <- shiny::reactive(
        x = {
      ...
      getCohortOverlapDataFiltered <- reactive(
        x = {
      ...
      getCharacterizationTableData <- shiny::reactive(
        x = {
          if (input$tabs != "cohortCharacterization")
          {
            return(NULL)
      ...
      getCharacterizationTableDataPretty <- shiny::reactive(
        x = {
          if (input$tabs != "cohortCharacterization")
          {
            return(NULL)
          ...
          expr = {
            ...
            options = list(
              ...
              columnDefs = list(truncateStringDef(0, 80),
                                minCellPercentDef(1:(
                                  length(
                                    temporalCovariateChoicesSelected$choices
                                  ))))
            )
        ...
        DT::renderDataTable(
          expr = {
            ...
            comparatorcohortIdValue <- balance %>%
              ...
              unique()
            databaseIdForCohortCharacterization <- balance$databaseId %>%
              unique()
      ...
      getDataSourceInformation <- shiny::reactive(
        x = {
          data <- database
      ...
      getAllStartTimeFromMetadata <- shiny::reactive(
        x = {
      ...
      ##getMetadataParsed----
      getMetadataParsed <- shiny::reactive(
        x = {
          data <- list()
          metadataInformation <- getMetadataInformation()
      ...
      shiny::observeEvent(
        input$cohortCharacterizationInfo,
        {
          showInfoBox(
      ...
      shiny::observeEvent(
        input$temporalCharacterizationInfo,
        {
          showInfoBox(
            "Temporal Characterization",
      ...
      shiny::observeEvent(
        input$cohortOverlapInfo,
        {
          showInfoBox("Cohort Overlap", "html/cohortOverlap.html")
        })
      
      shiny::observeEvent(
        input$compareCohortCharacterizationInfo,
        {
          showInfoBox(
      ...
      output$cohortCountsCategories <-
        shiny::renderUI({
          ...
          tags$div(
            ...
            tags$div(
              if (length(cohortSubjectCount100To2500) > 0)
              {
                buildCohortConditionTable(
            ...
            tags$div(
              if (length(cohortSubjectRecordRatioEq1) > 0)
              {
                tags$p(
                  paste0(
                    scales::percent(
            ...
            tags$div(
              if (length(cohortSubjectRecordRatioGt1) > 0)
              {
                tags$p(
                  paste0(
                    "    ",
            ...
            tags$br(),
            tags$div(
              if (length(cohortsWithLowestSubjectConts) > 0)
              {
                buildCohortConditionTable(
            ...
            tags$br(),
            tags$div(
              if (length(cohortsWithHighestSubjectConts) > 0)
              {
                buildCohortConditionTable(
