...
shinyServer(function(input, output, session) {
  ...
  queryResult <- reactive({
    if (is.null(input$domains)) {
      ...
      sql <- SqlRender::render(sql = eventSql, 
      ...
      events <- DatabaseConnector::querySql(conn, sql, snakeCaseToCamelCase = TRUE)
  ...
  filteredEvents <- reactive({
    ...
    if (nrow(events) != 0 && input$filterRegex != "") {
      events <- events[grepl(input$filterRegex, events$conceptName, ignore.case = TRUE) |
                         events$domain == "Visit" |
  ...
  colorScale <- reactive({
    ...
    if (length(domains) == 1) {
      ...
      temp <- RColorBrewer::brewer.pal(max(3, length(domains) - 1), "Set2")
  ...
  output$eventTable <- renderDataTable({
    ...
    events$y <- NULL
    cohortsOfSubject <- which(cohort$subjectId == subjectIds[subject$index])
    for (i in cohortsOfSubject) {
      events <- rbind(events, data.frame(domain = "Cohort",
  ...
  plot <- reactive({
    ...
    if (nrow(events) == 0) {
      ...
      events$text <- sprintf("%s - %s<br>%s<br>%s", events$startDate, events$endDate, events$conceptName, events$typeConceptName)
      ...
      first <- TRUE
      for (i in cohortsOfSubject) {
        data <- data.frame(date = rep(cohort$cohortStartDate[i], 2), 
                           y = rep(yRange, 2),
                           text = sprintf("%s - %s", cohort$cohortStartDate[i], cohort$cohortEndDate[i]))
  ...
  observeEvent(input$filterInfo, {
    showModal(modalDialog(
      ...
      footer = NULL,
      size = "l",
      HTML("Filter the concept to include in the plot and table by concept name using a regular expression. 
