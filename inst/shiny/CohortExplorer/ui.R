...
shinyUI(
  fluidPage(
    titlePanel(sprintf("Exploring cohort %s in %s.%s", shinySettings$cohortDefinitionId, shinySettings$cohortDatabaseSchema, shinySettings$cohortTable)),
    fluidRow(
      column(2,
             ...
             checkboxGroupInput("domains", label = "Domains", choices = c("Condition era", "Condition occurrence", "Drug era", "Drug exposure", "Procedure", "Measurement", "Observation", "Visit", "Observation period")),
             textAreaInput("filterRegex", div("Concept Name Filter", actionLink("filterInfo", "", icon = icon("info-circle"))), placeholder = "regex"),
             checkboxInput("showPlot", "Show Plot", value = TRUE),
             checkboxInput("showTable", "Show Table", value = TRUE),
      ),
      column(10,
             conditionalPanel(condition = "input.showTable==1",
