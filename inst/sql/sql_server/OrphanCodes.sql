...
FROM (
	SELECT concept_id,
		...
		concept_name_terms,
		row_number() OVER (
			PARTITION BY concept_id ORDER BY concept_name_terms ASC,
...
FROM @vocabulary_database_schema.concept c1
INNER JOIN #search_sub_str ss1
	ON ss1.concept_id != c1.concept_id
		AND LOWER(c1.concept_name) = LOWER(ss1.concept_name)
ORDER BY ss1.concept_id,
	c1.concept_id;

-- add the orphan codes to concept id universe table
INSERT INTO @concept_id_universe
SELECT DISTINCT orphan_concept_id concept_id
