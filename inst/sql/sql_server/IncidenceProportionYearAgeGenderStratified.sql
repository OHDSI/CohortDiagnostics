...
INNER JOIN (
	SELECT DISTINCT index_year
	FROM #inc_num
	) iy
	ON YEAR(DATEADD(DAY, @min_observation_time, op.observation_period_start_date)) <= iy.index_year
...
TRUNCATE TABLE #inc_num;
DROP TABLE #inc_num;

TRUNCATE TABLE #inc_denom;
DROP TABLE #inc_denom;

{@first_occurrence_only} ? {
TRUNCATE TABLE #inc_denom_exclude;
DROP TABLE #inc_denom_exclude;
}