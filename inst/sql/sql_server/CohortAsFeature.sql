	...
	FROM (
		SELECT subject_id,
			...
			row_number() OVER (
				...
				subject_id ORDER BY cohort_start_date
				) AS rn
...
HAVING min(abs(DATEDIFF(dd, a.cohort_start_date, b.cohort_end_date))) = 
abs(DATEDIFF(dd, a.cohort_start_date, b.cohort_end_date));
