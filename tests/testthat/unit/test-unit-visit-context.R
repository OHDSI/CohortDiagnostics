...
test_that("classifyVisitType correctly classifies Inpatient visits", {
  expect_equal(CohortDiagnostics:::classifyVisitType(9201), "Inpatient")
})

test_that("classifyVisitType correctly classifies Outpatient visits", {
  expect_equal(CohortDiagnostics:::classifyVisitType(9202), "Outpatient")
})

test_that("classifyVisitType correctly classifies Emergency Room visits", {
  expect_equal(CohortDiagnostics:::classifyVisitType(9203), "Emergency Room")
})

test_that("classifyVisitType correctly classifies other visit types", {
  expect_equal(CohortDiagnostics:::classifyVisitType(9204), "Long-term care")
  expect_equal(CohortDiagnostics:::classifyVisitType(1234), "Other")
  expect_equal(CohortDiagnostics:::classifyVisitType(NA), "Other")
})

# --- Duration Calculation Tests ---

test_that("calculateVisitDuration calculates multi-day visits correctly", {
  expect_equal(CohortDiagnostics:::calculateVisitDuration("2020-01-01", "2020-01-05"), 4)
  expect_equal(CohortDiagnostics:::calculateVisitDuration("2020-01-01", "2020-01-11"), 10)
})

test_that("calculateVisitDuration handles same-day visits (duration = 0)", {
  expect_equal(CohortDiagnostics:::calculateVisitDuration("2020-01-01", "2020-01-01"), 0)
})

test_that("calculateVisitDuration handles Date objects", {
  start <- as.Date("2020-02-01")
  end <- as.Date("2020-02-10")
  expect_equal(CohortDiagnostics:::calculateVisitDuration(start, end), 9)
})
...
test_that("aggregateVisitContext applies min cell count suppression", {
  ...
  result <- CohortDiagnostics:::aggregateVisitContext(mockData, minCellCount = 5)
...
test_that("aggregateVisitContext handles empty data", {
  result <- CohortDiagnostics:::aggregateVisitContext(dplyr::tibble())
  expect_equal(nrow(result), 0)

  result2 <- CohortDiagnostics:::aggregateVisitContext(NULL)
...
test_that("Duration calculation handles missing or NA dates", {
  ...
  expect_true(is.na(CohortDiagnostics:::calculateVisitDuration(NA, "2020-01-01")))
  expect_true(is.na(CohortDiagnostics:::calculateVisitDuration("2020-01-01", NA)))
})
