...
The CohortDiagnostics package allows one to generate a wide set of diagnostics to evaluate cohort definitions against a database in the Common Data Model (CDM). These diagnostics include incidence rates (optionally stratified by age, gender, and calendar year), cohort characteristisc (comorbidities, drug use, etc.), and the codes found in the data triggering the various rules in the cohort definitions.
...
- The diagnostics data should be placed in a subfolder that should be called `data`. In this data folder, either place all the indivual zip files generated for each database, or (recommended) place the premerged .RDA file here.
