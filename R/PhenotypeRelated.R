...
loadAndExportPhenotypeDescription <- function(packageName,
  ...
  pathToCsv <-
    system.file("settings", phenotypeDescriptionFile, package = packageName)
  if (file.exists(pathToCsv)) {
    ParallelLogger::logInfo("Found phenotype description file. Loading.")
    
    checkInputFileEncoding(pathToCsv)
    
    phenotypeDescription <- readr::read_csv(
      file = pathToCsv,
    ...
    checkmate::assertNames(
      x = colnames(phenotypeDescription),
    ...
    phenotypeDescription <- phenotypeDescription %>%
      ...
      dplyr::mutate(
        phenotypeName = dplyr::coalesce(as.character(.data$phenotypeName), ""),
    ...
    ParallelLogger::logInfo(
      sprintf(
        "Phenotype description file has %s rows. Matching with submitted cohorts",
    ...
    if (nrow(phenotypeDescription) > 0) {
      writeToCsv(phenotypeDescription,
                 file.path(exportFolder, "phenotype_description.csv"))
    } else {
      warning(
        "Phentoype description csv file found, but records dont match the referent concept ids of the cohorts being diagnosed."
      )
    }
