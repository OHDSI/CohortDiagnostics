...
runConceptSetDiagnostics <- function(connection = NULL,
  ...
  if (runOrphanConcepts) {
    ...
    ParallelLogger::logInfo(paste0("Working on dbms = ", connection@dbms))
...
getOrphanConcepts <- function(connectionDetails = NULL,
  ...
  ParallelLogger::logInfo("done 1")
  searchStringData <- DatabaseConnector::renderTranslateQuerySql(connection = connection,
                                                    sql = "SELECT * FROM #string_not_long;", 
                                                    snakeCaseToCamelCase = TRUE) %>% 
    dplyr::tibble()
  
  codesetIds <- searchStringData$codesetId %>% unique()
  if (length(codesetIds) == 0) {
    return(NULL)
  }
  ParallelLogger::logInfo("done 2")
  for (i in (1:length(codesetIds))) {
    
    ParallelLogger::logInfo(paste0("Working on codeset id ", codesetIds[[i]], " on ", Sys.time()))
    
    
    searchStrings <- searchStringData %>% 
      dplyr::filter(.data$codesetId == codesetIds[[i]]) %>%
      dplyr::filter(stringr::str_detect(string = .data$stringNotLong,
                                        pattern = "'", 
                                        negate = TRUE)) %>% 
      dplyr::pull(.data$stringNotLong) %>% 
      tolower() %>% 
      unique()
    if (length(searchStrings) > 0) {
      
      sql1 <- "INSERT INTO #orphan_concept_table
              SELECT @codeset_id codeset_id,
              	s.concept_id
              FROM (
              	SELECT DISTINCT
              		c1.concept_id,
              	...
              	WHERE "
      sql2 <- paste0(paste0("LOWER(c1.concept_name) LIKE CONCAT ('%', '",
                     searchStrings,
              			"' , '%') "), collapse = "OR 
              			")
      sql3 <- " AND c1.invalid_reason IS NULL
              	...
              	SELECT DISTINCT
              		cs.concept_id,
              		cs.concept_synonym_name concept_name
              	FROM @vocabulary_database_schema.concept_synonym cs
              	WHERE "
      sql4 <- paste0(paste0("LOWER(concept_synonym_name) LIKE CONCAT ('%', '",
                            searchStrings,
                            "' , '%') "), collapse = "OR 
              			")
      sql5 <- ") s
              LEFT JOIN (
              	...
              	) ss ON ss.concept_id = s.concept_id
              WHERE ss.concept_id IS NULL;"
  ...
  writeLines("!!!!!!!!!!!!COMPLETED ORPHAN CODES - THEN WHERE IS THE PROBLEM!!!!!!!!!!!!")
  return(orphanCodes)
}
