...
runConceptSetDiagnostics <- function(connection = NULL,
  ...
  conceptSetDiagnosticsResults$conceptResolved <-
    resolveConceptSets(
      ...
      vocabularyDatabaseSchema = vocabularyDatabaseSchema,
      tempEmulationSchema = tempEmulationSchema,
      resolvedConceptsTable = "#resolved_concept_set",
      #ensures the temp table is usable
  ...
  sql <-
    " WITH concept_id_table (concept_id)
      ...
      WHERE a.concept_id_1 IN (SELECT concept_id FROM concept_id_table)
        AND a.concept_id_2 IN (SELECT concept_id FROM concept_id_table)
  ...
  sql <-
    " WITH concept_id_table (concept_id)
      AS (
    	...
    	WHERE a.ancestor_concept_id IN (SELECT concept_id from concept_id_table)
    	  AND a.descendant_concept_id IN (SELECT concept_id from concept_id_table)
  ...
  for (start in seq(1, length(sql), by = batchSize)) {
    ...
    sqlSubset <-
      sprintf("SELECT *\nINTO %s\nFROM (\n %s\n) tmp;",
              ...
              sqlSubset)
    sqlSubset <-
      paste0("DROP TABLE IF EXISTS ", tempTable, "; ", sqlSubset)
    sqlSubset <-
...
getOrphanConcepts <- function(connectionDetails = NULL,
  ...
  OrphanCodesSql <- SqlRender::loadRenderTranslateSql(
    "OrphanCodes.sql",
  ...
  searchStringData <-
    DatabaseConnector::renderTranslateQuerySql(connection = connection,
                                               sql = "SELECT * FROM #search_strings;",
                                               snakeCaseToCamelCase = TRUE) %>%
    dplyr::tibble() %>% 
    dplyr::arrange(.data$codesetId)
  
  # Create list of search strings (from concept names and synonyms), discarding those
  # short than 5 and longer than 50 characters
  # fonts changed to lower case
  # limit to top 1000 based on length per codesetId
  searchStringData <- searchStringData %>%
    dplyr::mutate(conceptName = tolower(conceptName)) %>%
    dplyr::mutate(conceptNameLength = nchar(conceptName)) %>%
    dplyr::filter(conceptNameLength >= 5) %>%
    dplyr::filter(conceptNameLength <= 50) %>%
    dplyr::group_by(.data$codesetId) %>%
    dplyr::arrange(desc(conceptNameLength), .by_group = TRUE) %>%
    dplyr::mutate(rn = dplyr::row_number()) %>%
    dplyr::filter(.data$rn <= 1000) %>%
    dplyr::select(-.data$rn)
  
  # find sub-string in long-string for each codesetId
  searchStringDataLong <- searchStringData %>%
    dplyr::inner_join(
      searchStringData %>%
        dplyr::rename(
          conceptNameLengthShort = conceptNameLength,
          conceptNameShort = conceptName
        ),
      by = "codesetId"
    ) %>%
    dplyr::filter(!is.na(.data$conceptNameLengthShort)) %>% 
    dplyr::filter(!is.na(.data$conceptNameLength)) %>% 
    dplyr::filter(.data$conceptNameLengthShort < .data$conceptNameLength) %>%
    dplyr::filter(stringr::str_detect(
      string = stringr::fixed(.data$conceptName),
      pattern = stringr::fixed(.data$conceptNameShort)
    )) %>%
    dplyr::select(.data$codesetId, .data$conceptName)
  
  #Discard longer string from search_string
  searchStringDataWithoutLongString <- searchStringData %>%
    dplyr::left_join(
      searchStringDataLong %>%
        dplyr::select(.data$codesetId,
                      .data$conceptName) %>% 
        dplyr::mutate(source = 2),
      by = c("codesetId", "conceptName")
    ) %>% 
    dplyr::filter(is.na(.data$source)) %>% 
    dplyr::select(-.data$source)
  
  codesetIds <- searchStringDataWithoutLongString$codesetId %>% unique()
  ...
  for (i in (1:length(codesetIds))) {
      ParallelLogger::logInfo(paste0("  - ", 
                                      scales::comma(i),
                                      " out of ",
                                      scales::comma(length(codesetIds)),
                                     " unique concept sets."))
    
    searchStrings <- searchStringDataWithoutLongString %>%
      dplyr::filter(.data$codesetId == codesetIds[[i]]) %>%
      dplyr::filter(stringr::str_detect(
        string = .data$conceptName,
        pattern = "'",
        negate = TRUE
      )) %>%
  ...
  dropOrphanConceptTempTables <- SqlRender::loadRenderTranslateSql(
  ...
  DatabaseConnector::executeSql(
    connection = connection,
    sql = dropOrphanConceptTempTables,
    profile = FALSE,
    progressBar = FALSE,
    reportOverallTime = FALSE
  )
...
getConceptOccurrenceRelativeToIndexDay <- function(cohortIds,
  ...
  # for all concepts in concept tracking table, get their descendants and their mapped concepts
  sqlVocabulary <-
  ...
  if (!is.null(conceptTrackingTable)) {
    # keeping track - all the conceptId used in index event breakdown diagnostic
    DatabaseConnector::renderTranslateExecuteSql(
      connection = connection,
      sql =  "INSERT INTO @concept_tracking_table
  ...
  sqlConceptIdCount <- "INSERT INTO #indx_breakdown
                        SELECT cohort_definition_id cohort_id,
                        	...
                        	'@domain_concept_id_short' domain_field,
                        	0 mapping,
  ...
  #conceptId is from standard_concept_id field of domain table and coConceptId is also from standard_concept_id field of same domain table
  ...
  sqlConceptIdCoConceptIdSameCount <- " INSERT INTO #indx_breakdown
                                        ...
                                        WHERE d1.@domain_concept_id != d2.@domain_concept_id -- self join when conceptId are not same
  ...
  if (runIndexDateConceptCoOccurrence) {
    ParallelLogger::logInfo("      - concept Co-occurrence matrix computation has been requested. This may take time")
  }
  ...
  for (j in (1:length(runBreakdownIndexEventRelativeDays))) {
    ...
    for (i in (1:nrow(domains))) {
      ...
      DatabaseConnector::renderTranslateExecuteSql(
        ...
        domain_concept_id_short = rowData$domainConceptIdShort, 
        reportOverallTime = FALSE,
        progressBar = FALSE
      )
      ...
      if (all(
        ...
        DatabaseConnector::renderTranslateExecuteSql(
          ...
          days_relative_index = runBreakdownIndexEventRelativeDays[[j]],
          domain_table_short = rowData$domainTableShort, 
          domain_concept_id_short = rowData$domainSourceConceptIdShort, 
          min_subject_count = minCellCount,
          reportOverallTime = FALSE,
          progressBar = FALSE
        )
